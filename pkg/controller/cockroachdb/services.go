...
func init() {
	...
	info := NewInfo(PublicServiceForCockroachDB, createIfNotExist, &corev1.Service{})
	info.Postfix = "-public"
	err := Register(PublicServiceHandler, info)
	if err != nil {
		panic(err.Error())
	}

	// Register RoleBinding
	info = NewInfo(ServiceForCockroachDB, createIfNotExist, &corev1.Service{})
	err = Register(ServiceHandler, info)
	if err != nil {
		panic(err.Error())
	}
}
